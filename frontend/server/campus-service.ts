...
function complete(db: D1Database, now: number) {
  // The status guard makes the reward exactly-once. Rating is a sum of this ledger.
...
export async function book(db: D1Database, user: User, input: Record<string, unknown>, now: number) {
  ...
  // A single conditional INSERT serializes conflict and rating checks with the write.
  // Repeating the same request ID never creates another reservation.
  const results = await db.batch<Row>([
    complete(db, now),
...
export async function cancel(db: D1Database, user: User, id: unknown, now: number) {
  ...
  const results = await db.batch<Row>([
    complete(db, now),
    // Monthly allowance is checked in the UPDATE, not in a preceding read.
